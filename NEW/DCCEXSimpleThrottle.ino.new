...
// /// @brief Create the menu screen for displaying the various menus
// /// @param initialMenu Pointer to the initial screen to be displayed
// /// @return MenuScreen object, unused in this context
// MenuScreen menuScreen(&serverMenu);

const uint8_t menuCount = 3;

Menu *serverMenu = new Menu("Select server");
Menu *selectLocoMenu = new Menu("Select loco");
Menu *actionMenu = new Menu("Select action");

Menu *menuList[menuCount] = {serverMenu, selectLocoMenu, actionMenu};
...
void setup() {
  ...
  // controller.begin();
  // controller.addScreen(&menuScreen);
  // display.displayStartupInfo(VERSION);
  appConfig.initialise(menuList, menuCount);
  appConfig.getDisplay()->displayStartupInfo(VERSION);
  appConfig.getController()->begin();
}
